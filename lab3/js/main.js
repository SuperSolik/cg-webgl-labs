...
function createFigures(gl) {
  ...
  // var colors1 = Array(vertices1.length / 3).fill(COLORS.GLASS).flat();
  // var colors2 = Array(vertices2.length / 3).fill(COLORS.GLASS).flat();
  // var colors3 = Array(vertices3.length / 3).fill(COLORS.GLASS).flat();
  // var colors4 = Array(vertices4.length / 3).fill(COLORS.GLASS).flat();
  // var colors5 = Array(vertices5.length / 3).fill(COLORS.COLUMNS).flat();
  // var colors6 = Array(vertices6.length / 3).fill(COLORS.COLUMNS).flat();
  // var colors7 = Array(vertices7.length / 3).fill(COLORS.COLUMNS).flat();
  // var colors8 = Array(vertices8.length / 3).fill(COLORS.COLUMNS).flat();
  // var colors9 = Array(vertices9.length / 3).fill(COLORS.CAPS).flat();
  // var colors10 = Array(vertices10.length / 3).fill(COLORS.CAPS).flat();
  // var colors11 = Array(vertices11.length / 3).fill(COLORS.CAPS).flat();
  // var colors12 = Array(vertices12.length / 3).fill(COLORS.CAPS).flat();
  // var colors13 = Array(vertices13.length / 3).fill(COLORS.CAPS).flat();
  // var colors14 = Array(vertices14.length / 3).fill(COLORS.CAPS).flat();
  const sceneObjects = {
    glass: {
      figures: [
...
function drawScene(gl, programInfo, sceneObjects) {  
  ...
  // mat4.fromRotation(cameraMatrix, 0, [0, 0, 0]);
  ...
  for(const [groupName, group] of Object.entries(sceneObjects)){
    for(const f of group.figures){
      ...
      uniformData.material = group.material;
      uniformData.color = group.material.ambient.slice(0, 3);
  ...
  window.requestAnimationFrame(() => {
    updateState();
    drawScene(gl, programInfo, sceneObjects)
  });
}
...
window.onload = () => {
  ...
  const lSwitch = document.getElementById("lSwitch");
  ...
  gl.clearColor(0, 0, 0, 1);                      // установить в качестве цвета очистки буфера цвета чёрный, полная непрозрачность
  ...
  let programInfo = {
    ...
    uniformLocations: {
      useLighting: gl.getUniformLocation(shaderProgram, 'lighting'),
      vertexColor: gl.getUniformLocation(shaderProgram, 'uVertexColor'),
      projectionMatrix: gl.getUniformLocation(shaderProgram, 'uProjectionMatrix'),
      modelViewMatrix: gl.getUniformLocation(shaderProgram, 'uModelViewMatrix'),
      lights: [
        {
          diffuse: gl.getUniformLocation(shaderProgram, 'lights[0].diffuse'),
          ambient: gl.getUniformLocation(shaderProgram, 'lights[0].ambient'),
          position: gl.getUniformLocation(shaderProgram, 'lights[0].position'),
        }
      ],
      material: {
        diffuse: gl.getUniformLocation(shaderProgram, 'material.diffuse'),
        ambient: gl.getUniformLocation(shaderProgram, 'material.ambient'),
      }
    },
  };

  const sceneObjects = createFigures(gl);
  ...
  lSwitch.addEventListener('change', () => {
      useLighting = lSwitch.checked;
  });

  lAmbient.addEventListener('input', (e) => {
    const a = e.target.value / 100;
    viewState.light.ambient = [a, a, a, 0];
  });

  lDiffuse.addEventListener('input', (e) => {
    const d = e.target.value / 100;
    viewState.light.diffuse = [d, d, d, 0];
  });
