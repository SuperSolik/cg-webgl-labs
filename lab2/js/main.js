...
window.onload = () => {
  ...
  const orthoSwitch = document.getElementById("oSwitch");
  ...
  const figures = createFigures(gl);

  orthoSwitch.addEventListener('change', () => {
    if(orthoSwitch.checked) {
  ...
  pSwitch.addEventListener('change', () => {
    if(pSwitch.checked) {
      currentPerspective = PERSPECTIVE.PROJECTION;
      orthoSwitch.checked = false;
    } else {
      currentPerspective = PERSPECTIVE.ORTHO;
      orthoSwitch.checked = true;
    }
  });
